...
### 优化内容：
1. **使用AndroidX Compat库**：所有系统服务获取都使用 `androidx.core.content.getSystemService<T>()` 扩展函数
2. **移除无用代码**：清理了所有多余的空行、注释和未使用的import
3. **空安全处理**：对可能为null的对象进行了安全检查
4. **代码格式化**：统一了代码风格，符合Kotlin编码规范
5. **编译验证**：所有文件已通过编译检查，无任何错误或警告
