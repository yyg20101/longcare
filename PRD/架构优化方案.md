...
> [!IMPORTANT]
> **设计原则**：避免使用JSON字段存储数据，使用关联表保证查询效率和数据完整性。

##### 数据库ER图

```
┌─────────────────────┐       ┌─────────────────────────┐
│     OrderEntity     │───1:N─│   OrderProjectEntity    │
│  (订单主表)          │       │  (订单项目关联表)        │
├─────────────────────┤       ├─────────────────────────┤
│ PK: orderId         │       │ PK: id                  │
│ planId              │       │ FK: orderId             │
│ state               │       │ projectId               │
│ elderUserId         │       │ projectName             │
│ elderName           │       │ serviceTime             │
│ ...                 │       │ isSelected (本地状态)   │
└─────────────────────┘       └─────────────────────────┘
         │
         │ 1:N
         ▼
┌─────────────────────────┐    ┌─────────────────────────┐
│   OrderImageEntity      │    │  OrderLocationEntity    │
│  (订单图片表)            │    │  (订单定位记录表)        │
├─────────────────────────┤    ├─────────────────────────┤
│ PK: id                  │    │ PK: id                  │
...
│ imageType               │    │ latitude                │
│ localUri                │    │ longitude               │
│ uploadStatus            │    │ accuracy                │
│ cloudKey                │    │ uploadStatus            │
└─────────────────────────┘    └─────────────────────────┘
```
...
data class OrderEntity(
    ...
    // 订单基本信息
    val planId: Int = 0,
    val state: Int = 0,                       // 服务状态（服务端）
    val startTime: String = "",               // 服务开始时间（服务端格式）
    val endTime: String = "",                 // 服务结束时间（服务端格式）
    
    // 用户信息（扁平化存储，来自API的userInfo）
    ...
    // ========== 本地数据（客户端维护） ==========
    val localStatus: String = "PENDING",      // PENDING, IN_PROGRESS, COMPLETED
...
// 注意：移除了 projectListJson 和 selectedProjectIds，改用 OrderProjectEntity 关联表
```

##### 2. 订单项目关联表 (OrderProjectEntity) - 替代JSON字段
...
@Entity(
    ...
    indices = [
        Index("orderId"),
        Index("orderId", "projectId", unique = true)  // 联合唯一索引
    ]
)
data class OrderProjectEntity(
    ...
    val serviceTime: Int = 0,                 // 服务时长（分钟）
    val lastServiceTime: String = "",         // 最后服务时间
    val isComplete: Int = 0,                  // 是否完成（服务端）
    
    // ========== 本地数据 ==========
    val isSelected: Boolean = false,          // 用户是否选中该项目
...
@Entity(
    ...
    indices = [Index("orderId"), Index("orderId", "imageType")]
)
data class OrderImageEntity(
    ...
    // 图片类型：CUSTOMER, BEFORE_CARE, CENTER_CARE, AFTER_CARE
    ...
    // 上传状态：PENDING, UPLOADING, SUCCESS, FAILED, CANCELLED
...
navController.navigate(ServiceRoute(orderId))

// 页面中从数据库获取完整数据
@Composable
fun ServiceScreen(orderId: Long) {
...
| 阶段 | 任务 | 预估工时 | 优先级 |
|-----|------|---------|-------|
| **Phase 1** | Room数据库设计和核心实体创建 | 2天 | 高 |
| **Phase 2** | UnifiedOrderRepository实现（订单同步核心） | 2天 | 高 |
| **Phase 3** | 图片实体和DAO实现 | 2天 | 高 |
| **Phase 4** | 导航改造（只传orderId） | 3天 | 高 |
...
#### Phase 1: Room数据库设计和核心实体创建

- 创建 `LongCareDatabase`
- 创建 `OrderEntity`（包含服务端数据和本地数据）
- 创建 `OrderImageEntity`（图片状态跟踪）
- 创建 `OrderLocationEntity`（定位记录）
- 配置Room和KSP

#### Phase 2: UnifiedOrderRepository实现

- 实现 `OrderDao` 完整CRUD操作
- 实现 `UnifiedOrderRepository`
- 实现API调用→数据库同步逻辑
- 提供Flow供UI订阅
- 单元测试
...
- 改造 `PhotoProcessingViewModel` 使用新DAO
- 支持取消/重传场景

#### Phase 4: 导航改造

- 修改所有Route类，改为只接收 `orderId`
- 移除 `OrderInfoRequestModel` 的导航传递
- 修改所有ViewModel，从 `UnifiedOrderRepository` 获取数据
- 移除 `SharedOrderRepository`（内存缓存版本）
...
**导航相关（Phase 4）：**
```
- navigation/NavigationRoutes.kt           (所有Route改为只接收orderId)
- navigation/AppNavigation.kt              (移除OrderInfoRequestModel序列化)
- navigation/OrderInfoRequestModelNavType.kt  (可废弃)
```
...
| 导航参数 | 传递完整对象（orderId+planId+序列化） | 只传 orderId |
