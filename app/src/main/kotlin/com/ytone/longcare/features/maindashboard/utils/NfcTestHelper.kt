...
class NfcTestHelper @Inject constructor(
    ...
    private fun startNfcListening(activity: Activity) {
        ...
        when {
            !NfcUtils.isNfcSupported(activity) -> {
                logD(NfcTestConfig.TEST_TAG, "设备不支持NFC")
                toastHelper.showShort("设备不支持NFC功能")
                return
            }
            !NfcUtils.isNfcEnabled(activity) -> {
                logD(NfcTestConfig.TEST_TAG, "NFC未开启")
        ...
        try {
            ...
            if (activity is LifecycleOwner) {
                ...
                eventJob = activity.lifecycleScope.launch {
                    logD(NfcTestConfig.TEST_TAG, "事件监听协程已启动")
                    appEventBus.events.collect { event ->
                        logD(NfcTestConfig.TEST_TAG, "收到事件: ${event::class.java.simpleName}, isListening: $isListening")
                        if (event is AppEvent.NfcIntentReceived && isListening) {
                            logD(NfcTestConfig.TEST_TAG, "处理NFC事件: ${event.intent.action}")
                            handleNfcIntent(event.intent)
                        } else if (event is AppEvent.NfcIntentReceived) {
                            logD(NfcTestConfig.TEST_TAG, "收到NFC事件但isListening为false")
                        }
                    }
                }
    ...
    private fun handleNfcIntent(intent: Intent) {
        try {
            logD(NfcTestConfig.TEST_TAG, "处理NFC Intent: ${intent.action}")
            ...
            if (tag != null) {
                ...
                logD(NfcTestConfig.TEST_TAG, "获取到Tag ID: $tagId")
                
                if (tagId.isNotEmpty()) {
                    // 显示弹窗
                    nfcTagId = tagId
                    showDialog = true
                    logD(NfcTestConfig.TEST_TAG, "已设置弹窗显示: showDialog=$showDialog, nfcTagId=$nfcTagId")
                } else {
                    toastHelper.showShort("无法获取NFC标签ID")
                }
            } else {
                logD(NfcTestConfig.TEST_TAG, "无法从 Intent 中获取 Tag")
                toastHelper.showShort("无法读取NFC标签")
            }
        } catch (e: Exception) {
            logD(NfcTestConfig.TEST_TAG, "处理NFC数据失败: ${e.message}")
    ...
    private fun copyToClipboard(text: String) {
        try {
            ...
            logD(NfcTestConfig.TEST_TAG, "已复制到剪贴板: $text")
            toastHelper.showShort("已复制到剪贴板")
        } catch (e: Exception) {
            logD(NfcTestConfig.TEST_TAG, "复制到剪贴板失败: ${e.message}")
     ...
     */
    fun copyAndDismiss() {
        logD(NfcTestConfig.TEST_TAG, "复制Tag ID: $nfcTagId")
    ...
    fun NfcTagDialog() {
        if (showDialog) {
            logD(NfcTestConfig.TEST_TAG, "显示NFC弹窗 - nfcTagId: $nfcTagId")
            AlertDialog(
                onDismissRequest = {
    ...
    override fun onResume(owner: LifecycleOwner) {
        ...
        logD(NfcTestConfig.TEST_TAG, "onResume - isEnabled: $isEnabled, isListening: $isListening")
    ...
    override fun onPause(owner: LifecycleOwner) {
        super.onPause(owner)
        logD(NfcTestConfig.TEST_TAG, "onPause - isListening: $isListening")
        currentActivity?.let { activity ->
            if (isListening) {
                stopNfcListening(activity)
            }
    ...
    override fun onDestroy(owner: LifecycleOwner) {
        ...
        logD(NfcTestConfig.TEST_TAG, "onDestroy")
        currentActivity?.let { activity ->
            disable(activity)
        }
    }
